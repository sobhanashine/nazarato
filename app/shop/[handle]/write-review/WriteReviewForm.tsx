...
import { submitShopReview, type WriteReviewState } from "./actions";
...
const PROOF_TYPES = [
  { value: "dm", label: "اسکرین‌شات پیام (DM)" },
  { value: "receipt", label: "فیش واریز / رسید پرداخت" },
];
...
export function ShopWriteReviewForm({
  handle,
  businessName,
}: {
  handle: string;
  ...
  const [state, action, pending] = useActionState(submitShopReview, initial);
  ...
  return (
    <div className={`${GLASS} p-6 sm:p-8`}>
      ...
      <form action={action} noValidate className="mt-6 space-y-6">
        <input type="hidden" name="slug" value={handle} />
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea
            ...
            placeholder="برخورد ادمین چطور بود؟ کیفیت کالا با عکس‌ها همخوانی داشت؟ ارسال به موقع انجام شد؟ برای راهنمایی بقیه جزئیات را بنویس."
        ...
        <div className="rounded-xl border border-glass-border bg-white/[0.02] p-4">
          ...
          <p className={`${HINT} mt-1`}>
            با بارگذاری اسکرین‌شات چت دایرکت یا فیش واریز، نظرت پس از یک بررسی خصوصی
            توسط ادمین به «نقد تأییدشده» ارتقا می‌یابد. بدون سند هم نظرت کاملاً
            معتبر است.
          </p>
