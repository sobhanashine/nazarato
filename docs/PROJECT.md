...
- **Auth — remaining wiring.** `/login` is built against Supabase, but:
  (1) the `supabase/migrations/0001_create_users_table.sql` migration must be
  run in the Supabase SQL editor before login works; (2) OTP delivery is a
  static dev code — wire Kavenegar in `lib/auth/otp.ts` `sendOtp`; (3) set
  `JWT_SECRET` in `.env.local` (an insecure fallback + console warning is used
  until then). Sessions are signed cookies — a deliberate placeholder; migrate
  to Supabase Auth later (keep `getSession`/`setSession`/`clearSession` stable).
